    ...
    def export(self):
        ...
        for date_as_date, list_bks in self.grouped_highlights.items():
            ...
            for book in list_bks:
                print(
                    f"Book:  {book.readable_title[:40]:40} Total Highlights: {len(list_bks)}"
                )
                bh_uid = self._decide(
                    self.state.rwh_state, rw_header_uid, book.roam_book_header, 3
                )
                b_summary_uid = self._decide(
                    self.state.book_state, bh_uid, book.roam_sub_header
                )
            ...
            for book in list_bks:
                ...
                header_uid = self._decide(
                    self.state.links_state, rw_links_uid, book_link_header
                )
